...
from models.MultiHead.GateAttention import (
    GatingSignal,
    AttentionBlock
)
...
class CommonNeck(nn.Module):
    def __init__(self, emb_shape, in_ch, size=32):
        self.in_ch = in_ch
        super(CommonNeck, self).__init__()
    ...
    def forward(self, x, emb_codes):
        ...
        return hid, [feat_2, feat_1, feat_0]
...
class Head(nn.Module):
    ...
    def forward(self, hid, features):
        feat_2, feat_1, feat_0 = features
...
class PosPadUNet3D(nn.Module):
    def __init__(self, n_classes, emb_shape, in_ch, size=32):
        ...
        self.neck = CommonNeck(emb_shape, in_ch, size)
        self.pulp_head = Head(n_classes, size)
        self.nerve_head = Head(n_classes, size)

    def forward(self, x, emb_codes):
        hid, features = self.neck(x, emb_codes)
        pulp_logits = self.pulp_head(hid, features)
        nerve_logits = self.nerve_head(hid, features)
        return pulp_logits, nerve_logits
