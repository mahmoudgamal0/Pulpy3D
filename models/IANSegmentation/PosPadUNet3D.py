...
from models.FairyPad.GridAttention import GridAttentionBlock3D_TORR as AttentionBlock3D
from models.FairyPad.GridAttention import UnetGridGatingSignal3 as AttentionGating3D
...
class PosPadUNet3D(nn.Module):
    def __init__(self, n_classes, emb_shape, in_ch, size=32):
        ...
        self.gating9 = AttentionGating3D(size*16+1, size*16, kernel_size=3, stride=1, is_batchnorm=True)
        self.gating6 = AttentionGating3D(size*8, size*8, kernel_size=3, stride=1, is_batchnorm=True)
        self.gating3 = AttentionGating3D(size*4, size*4, kernel_size=3, stride=1, is_batchnorm=True)

        self.attentionblock9 = AttentionBlock3D(in_channels=size*16+1, gating_channels=size*16)
        self.attentionblock6 = AttentionBlock3D(in_channels=size*8, gating_channels=size*8)
        self.attentionblock3 = AttentionBlock3D(in_channels=size*4, gating_channels=size*4)
        #################
    ...
    def forward(self, x, emb_codes):
        ...
        g_dc9, _ = self.attentionblock9(hid, gating9)
        ...
        g_dc6, _ = self.attentionblock6(hid, gating6)
        hid = torch.cat((self.dc6(g_dc6), feat_1), dim=1)
        hid = self.dc5(hid)
        hid = self.dc4(hid)

        gating3 = self.gating3(hid)
        g_dc3, _ = self.attentionblock3(hid, gating3)
